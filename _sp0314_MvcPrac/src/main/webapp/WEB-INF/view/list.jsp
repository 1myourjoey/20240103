            ...
            <c:forEach var="board" items="${list}">
                <tr>
                    ...
                    <td style="text-align:left;">
                        <a href="selectOne?num=${board.num}">
                            ${board.title}
                        </a>
                    </td>
                    <td>${board.writer}</td> <!-- 수정된 부분: 작성자 정보를 출력 -->
                    <td>${board.regtime}</td>
                    <td>${board.hits}</td>
                </tr>
            </c:forEach>
...
<div class="mt-4 btn-container text-center">
        <input type="button" class="btn btn-secondary" value="Write" onclick="location.href='write.jsp'"> <!-- 수정된 부분: Write 버튼 클릭 시 write.jsp로 이동 -->
</div>
